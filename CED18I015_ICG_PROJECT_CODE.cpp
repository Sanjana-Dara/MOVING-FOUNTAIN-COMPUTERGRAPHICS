...
GLint NumOfVerticesStone = 360;    //only a quarter of the finally used vertices
...
void Display(void)
{
    ...
    glLoadIdentity();
    //Load a new modelview matrix -> we can apply new transformations
...
int main(int argc, char **argv)
{
    ...
    glutAddMenuEntry("Stop Flow: u",1);
    glutAddMenuEntry("Stop rotation: t",2);
    glutAddMenuEntry("Stop translation: m",3);
    glutCreateMenu(CMain);
    glutAddSubMenu("Colors",sub_menu);
    glutAddSubMenu("Help",sub_menu4);
    glutAddSubMenu("Rays",sub_menu2);
    glutAddSubMenu("Steps",sub_menu3);
